...
EntityGuide = ig.Entity.extend({
	...
	init: function(x, y, settings) {
        ...
        if (ig.game.currentLevel === LevelMain) {
            if (ig.game.guideBuffer === 2) {
                this.guideContractionSpeed = 50;
                this.beatGuideSpeed = 0.5;
            }
            else if (ig.game.guideBuffer === 1) {
                this.guideContractionSpeed = 100;
                this.beatGuideSpeed = 1;
            }
        }
        ...
        switch (this.kind) {
            ...
            case 'beat':
                ...
                this.guideContractionSpeed = this.beatGuideSpeed * ig.system.height;
    ...
    update: function() {
        ...
        // If this is a beat guide
            ...
            // Kill when beat guide is too high
                ...
                ig.game.spawnEntity(EntityPoints, this.pos.x, this.pos.y + 150, {points: this.beatPoints});
                ig.game.controller.score += this.beatPoints;
                this.kill();
            }
            ...
            if (ig.input.pressed('space')) {
                if (this.kind === 'beat') {
                    if (this.pos.y <= 30) {
                        this.beatPoints = ig.game.controller.maxBeatPoints;
                    }
                }
            }
    ...
    draw: function() {
        ...
        if (this.kind !== 'beat') {
            ...
            ig.game.ctx.beginPath();
            ig.game.ctx.arc(this.pos.x, this.pos.y, this.expansionSpeed * this.duration, 0, 2 * Math.PI, false);
            ig.game.ctx.strokeStyle = 'rgba(' + this.gray.r + ',' + this.gray.g + ',' + this.gray.b + ',' + 0.5 + ')';
            ig.game.ctx.lineWidth = 1.5;
            ig.game.ctx.stroke();
