...
MyGame = ig.Game.extend({
	...
	// Load music
	virusMusic: new ig.Sound( 'media/virus.*',false ),
	titleMusic: new ig.Sound( 'media/titlesound.*',false ),
	...
	//	this.spawnEntity( EntityPointer, 0, 0 ); // Spawn cursor
		...
		if (this.currentLevel === LevelMain) {
			...
			this.beatGuideAlphaTimer = new ig.Timer(0.1);
		}
		else {
	...
	setTriggerMarks: function() {
		...
		this.triggerMarks = [
				...
				//	{mark: 141.93, duration: 0, kind: 'beat'},
					...
					{mark: 187.87, duration: 0.87, kind: 'red'},
					{mark: 189.51, duration: 0.87, kind: 'purple'},
					{mark: 191.27, duration: 0, kind: 'beat'},
					{mark: 191.63, duration: 0, kind: 'beat'},
					{mark: 192.06, duration: 0, kind: 'beat'},
					{mark: 192.48, duration: 0, kind: 'beat'},
					{mark: 193.65, duration: 0, kind: 'beat'},
					{mark: 194.54, duration: 1.68, kind: 'blue'},
					{mark: 197.82, duration: 0, kind: 'beat'},
					{mark: 198.18, duration: 0, kind: 'beat'},
					{mark: 198.63, duration: 0, kind: 'beat'},
					{mark: 199.04, duration: 0, kind: 'beat'},
					{mark: 200.29, duration: 0, kind: 'beat'},
					{mark: 200.93, duration: 1.78, kind: 'blue'},
					{mark: 202.71, duration: 0.67, kind: 'red'},
					{mark: 204.38, duration: 0, kind: 'beat'},
					{mark: 204.76, duration: 0, kind: 'beat'},
					{mark: 205.15, duration: 0, kind: 'beat'},
					{mark: 205.62, duration: 1.42, kind: 'green'},
					{mark: 209.32, duration: 0.86, kind: 'purple'},
					{mark: 210.98, duration: 1.17, kind: 'green'},
					{mark: 213.51, duration: 2.29, kind: 'red'},
					{mark: 215.80, duration: 0.87, kind: 'green'},
					{mark: 217.54, duration: 0, kind: 'beat'},
					{mark: 217.95, duration: 0, kind: 'beat'},
					{mark: 218.28, duration: 0, kind: 'beat'},
					{mark: 218.70, duration: 0, kind: 'beat'},
					{mark: 220.69, duration: 0, kind: 'beat'},
					{mark: 223.27, duration: 0, kind: 'beat'},
					{mark: 224.28, duration: 0.99, kind: 'red'},
					{mark: 227.25, duration: 3.56, kind: 'blue'},
					{mark: 234.01, duration: 0, kind: 'beat'},
					{mark: 235.62, duration: 0, kind: 'beat'}
		];
	},
	...
	setTutorialMark: function() {
		...
		if (this.currentLevel === LevelTitle) {
			var color = this.controller.randomFromTo(1,4); // Pick random trigger to spawn
			var kind;
			switch (color) {
				case 1: kind = 'red';
				break;
	...
	update: function() {
		...
		if (this.currentLevel === LevelTitle) {
			...
			if (ig.input.pressed('space')) {
				this.beatGuideAlpha = 0.5;
				this.beatGuideAlphaTimer.unpause();
			}

			if (this.beatGuideAlphaTimer.delta() > 0) {
				this.beatGuideAlpha = 0.3;
				this.beatGuideAlphaTimer.pause();
				this.beatGuideAlphaTimer.reset();
			}
	...
	drawTitleScreen: function() {
		...
		var y = 100;
		this.ctx.fillStyle = '#7e0302';
		this.ctx.font='19pt Arial';
		this.ctx.fillText('my mind is going', x, y);
		y += 30;
		this.ctx.fillStyle = '#cccccc';
		this.ctx.font='14.5pt Arial';
		this.ctx.fillText('space to start.', x, y);
		y += 40;
		
		this.ctx.fillStyle = '#7e0302';
		this.ctx.font='14.5pt Arial';
		this.ctx.fillText('how:', x, y);
		y += 30;
		this.ctx.font='11pt Arial';
		this.ctx.fillStyle = '#ffffff';
		this.ctx.fillText('Sustained Triggers:', x, y);
		y += 25;
		this.ctx.fillStyle = '#959595';
        x += 60;
        this.ctx.fillText('+ Press number key shown in guide when gray silhouette touches guide outline.', x, y);
        y += 20;
		this.ctx.fillText('+ Hold to expand as the guide expands.', x, y);
        y += 20;
		this.ctx.fillText('+ Release to contract when guide starts contracting.', x, y);

		x -= 60;
		y += 40;
		this.ctx.fillStyle = '#ffffff';
		this.ctx.fillText('Beat Triggers:', x, y);
		y += 15;
		this.ctx.fillStyle = '#959595';
		...
		this.ctx.fillRect(x, y, 433, 25);

		this.ctx.fillStyle = 'gray';
		this.ctx.font="19pt Arial";
		this.ctx.fillText('space', ig.system.width / 2 - 27, y + 18);
		this.ctx.restore();
		y += 40;
		this.ctx.fillText('Hit SPACE once when horizontal silhouette touches the guide.', x, y);
		y += 40;
		this.ctx.fillStyle = '#7e0302';
		this.ctx.font='17px Arial';
		this.ctx.fillText('Music by Clearside - www.clearsidemusic.com', x, y);

	},

	drawMainScreen: function() {
		this.ctx.beginPath();
		this.ctx.fillStyle = 'rgba(255,255,255,' + this.beatGuideAlpha + ')';
		this.ctx.fillRect(0, 0, ig.system.width, 25);

		this.ctx.fillStyle = 'gray';
		this.ctx.font="19pt Arial";
		this.ctx.fillText('space', ig.system.width / 2 - 27, 18);
		this.ctx.font="40pt Arial";
		this.ctx.fillText(this.controller.score, 5, ig.system.height - 55);

		// Draw progress bar
		this.ctx.beginPath();
		this.ctx.fillStyle = '#7e0302';
		this.ctx.fillRect(0, ig.system.height - 15, ig.system.width / 240.55 * ig.music.currentTrack.currentTime, 15);
	},
	...
	drawEndScreen: function() {
		...
		this.ctx.fillStyle = '#7e0302';
		this.ctx.font='19pt Arial';
		this.ctx.fillText('finished', x, y);
		y += 30;
		this.ctx.fillStyle = '#cccccc';
		this.ctx.font='14.5pt Arial';
		this.ctx.fillText('Score: ' + this.controller.score, x, y);
		y += 40;
		this.ctx.fillText('Space to try again.', x, y);
	},
